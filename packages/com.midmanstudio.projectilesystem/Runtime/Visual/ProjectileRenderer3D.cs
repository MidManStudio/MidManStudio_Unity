...
namespace MidManStudio.Projectiles.Visuals
{
    public sealed class ProjectileRenderer3D : MonoBehaviour
    {
        ...
        // Two separate meshes — sprite pass and shape pass never share one mesh
        ...
        private void Awake()
        {
            _spriteMesh = new Mesh { name = "ProjectileSprite3D" };
            _spriteMesh.MarkDynamic();
            _shapeMesh  = new Mesh { name = "ProjectileShape3D" };
        ...
        {
            if (_atlasMaterial == null || projs == null || count == 0) return;

            // Each pass writes into its own dedicated mesh
            RenderPass(projs, count, spritePass: true,  mesh: _spriteMesh);
            RenderPass(projs, count, spritePass: false, mesh: _shapeMesh);
        }
        ...
        private void RenderPass(
            NativeProjectile3D[] projs, int count, bool spritePass, Mesh mesh)
        {
            ...
            for (int i = 0; i < count && qi < _maxQuads; i++)
            {
                ...
                if (vel.sqrMagnitude > 0.0001f)
                {
                    ...
                    Vector3 camToProj = cam != null
                        ? (pos - cam.transform.position) : Vector3.back;
            ...
            // Upload into THIS pass's dedicated mesh only
            mesh.Clear();
            ...
            Graphics.DrawMesh(
                ...
                gameObject.layer, camera: null, submeshIndex: 0,
                properties: _mpb, castShadows: false, receiveShadows: false);
        }
